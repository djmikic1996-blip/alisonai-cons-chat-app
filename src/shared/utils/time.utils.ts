...
export const getRelativeTime = (currentTime: number, timestamp?: number): string => {
  if (timestamp === undefined) return '-';

  const seconds = Math.floor((currentTime - timestamp) / TIME_CONSTANTS.MS_PER_SECOND);

  if (seconds < TIME_CONSTANTS.JUST_NOW_THRESHOLD / TIME_CONSTANTS.MS_PER_SECOND) {
    return 'just now';
  }
